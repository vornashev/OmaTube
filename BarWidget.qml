...
BarWidget {
  ...
  function injectLogic() {
    if (!logic) return
    logic.bar = root.bar; logic.settings = root.settings; logic.anchorItem = playerLoader.item; logic.hostWidget = root
  }
  ...
  Loader { id: logicLoader; active: true; visible: false; source: Qt.resolvedUrl("WidgetLogic.qml"); onLoaded: { root.injectLogic(); Qt.callLater(root.injectLogic) } }
  Loader {
    id: playerLoader; anchors.fill: parent; active: true; source: Qt.resolvedUrl("BarPlayer.qml")
    onLoaded: { item.bar = root.bar; item.logic = root.logic; item.hostWidget = root; root.injectLogic() }
  }
  Connections {
    target: root
    function onBarChanged() { if (playerLoader.item) playerLoader.item.bar = root.bar }
    function onLogicChanged() { if (playerLoader.item) playerLoader.item.logic = root.logic; root.injectLogic() }
  }
}
