...
class Player:
    ...
    def status(self) -> dict[str, Any]:
        return {"state": self.state, "current": self.current, "position": self.position,
                ...
                "queueRevision": self.queue_revision, "error": self.error}
    ...
    async def _load_current(self, start: float = 0.0, retry: bool = True) -> None:
        ...
        async def resolve_and_load() -> None:
            try:
                audio = await self.resolver.resolve_audio(current["media"]["videoId"])
                ...
                await self.mpv.load(audio, start if audio.seekable else 0)
                if generation != self.generation: return
                await self.mpv.set_volume(self.volume); await self.mpv.set_mute(self.muted)
                self.state = "playing"; self.changed()
    ...
    async def play(self) -> None:
        ...
        if self.state in {"paused", "playing"} and self.mpv.writer:
    ...
    async def seek(self, seconds: float) -> None:
        ...
        await self.mpv.seek(seconds); self.position = max(0.0, seconds); self.persist(); self.changed()
    ...
    async def handle_mpv_event(self, event: dict[str, Any]) -> None:
        ...
        if event.get("event") == "property-change":
            ...
            elif name == "pause" and isinstance(value, bool) and self.current_id: self.state = "paused" if value else "playing"
            ...
            if reason == "eof":
                ...
                if self._retried_entry_id != self.current_id:
                    ...
                    self.state = "error"
                    self.error = {"code": "playback_error", "message": "Аудиопоток оборвался; повторите или пропустите элемент"}
                    self.changed()
